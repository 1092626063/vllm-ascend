...
In addition, if you don't want to use the docker image, you can also build all from source:
...
- Quantized model `DeepSeek-V3.1-w8a8` can be deployed on 1 Atlas 800 A3 (64G × 16).
...
- `DeepSeek-V3.1-w8a8`: require at least 2 Atlas 800 A2 (64G × 8).
...
Take Atlas 800 A3 (64G × 16) for example, we recommend to deploy 2P1D (4 nodes) rather than 1P1D (2 nodes), because there is no enough NPU memory to serve high concurrency in 1P1D case.
 - `DeepSeek-V3.1-w8a8 2P1D Layerwise` require 4 Atlas 800 A3 (64G × 16).

To run the vllm-ascend `Prefill-Decode Disaggregation` service, you need to deploy a `launch_dp_program.py` script and a `run_dp_template.sh` script on each node and deploy a `proxy.sh` script on prefill master node to foward requests.
...
2. After execution, you can get the result, here is the result of `DeepSeek-V3.1-W8A8` in `vllm-ascend:0.11.0rc0` for reference only.
